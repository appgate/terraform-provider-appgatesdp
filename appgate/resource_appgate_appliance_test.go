...
func TestAccApplianceConnectionBroker(t *testing.T) {
	resourceName := "appgatesdp_appliance.connector"
	rName := RandStringFromCharSet(10, CharSetAlphaNum)
	context := map[string]interface{}{
		"name":     rName,
		...
		Steps: []resource.TestStep{
			{
				...
				Config: testAccCheckApplianceBasicConnectionBroker(context),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckApplianceExists(resourceName),
					resource.TestCheckResourceAttr(resourceName, "client_interface.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "client_interface.0.allow_sources.#", "1"),
					...
					resource.TestCheckResourceAttr(resourceName, "connection_broker.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "connection_broker.0.enabled", "true"),
					resource.TestCheckResourceAttr(resourceName, "connection_broker.0.sites.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "connection_broker.0.sites.0", "8a4add9e-0e99-4bb1-949c-c9faf9a49ad4"),
					resource.TestCheckResourceAttr(resourceName, "name", rName),
...
func testAccCheckApplianceBasicConnectionBroker(context map[string]interface{}) string {
	return Nprintf(`
data "appgatesdp_site" "default_site" {
    site_name = "Default Site"
}
resource "appgatesdp_appliance" "connector" {
    ...
    connection_broker {
      enabled = true
      sites = [data.appgatesdp_site.default_site.id]
    }
}
`, context)
}
